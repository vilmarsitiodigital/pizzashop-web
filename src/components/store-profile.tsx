...
interface IProps {
  setOpenDialogAccount: (openDialogAccount: boolean) => void
}

export function StoreProfile({ setOpenDialogAccount }: IProps) {
  ...
  return (
    <DialogContent className="w-11/12 sm:w-[480px]">
      <DialogHeader>
      ...
      <form onSubmit={handleSubmit(handleUpdateProfile)}>
        ...
        <DialogFooter className="flex flex-row gap-2">
          <DialogClose asChild className="w-full">
            <Button
              onClick={() => setOpenDialogAccount(false)}
              variant="ghost"
              type="button"
            >
              Cancelar
